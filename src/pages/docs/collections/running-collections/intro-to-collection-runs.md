...
1. Choose any configuration options:
    ...
    * **Advanced settings**
      * **Stop run if an error occurs** - By default, the collection run stops if there's a problem getting a response from the server or the server returns an error in response to a request. Clear this checkbox if you want the collection run to continue after an error occurs.
