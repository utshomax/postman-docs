...
1. Choose any configuration options:
    ...
    * **Advanced settings**
      * **Stop run if an error occurs** - By default, the collection run stops if there's a problem sending a request to or getting a response from the server. Clear this checkbox if you want the collection run to continue after an error occurs. (This option does not apply to 4xx errors returned by the server.)
