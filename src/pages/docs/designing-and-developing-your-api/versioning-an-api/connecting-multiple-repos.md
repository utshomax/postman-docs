...
Make sure to select the correct folders for your API's schema and collections when connecting the API to the repository.
...
You must use a unique combination of branch and folder for each API you connect to the repository. For example, if you connect one API to the `/postman/schemas` folder on the `notifications-v1` branch, you can't connect another API to the same folder on the same branch. You must use a different branch or a different schema folder.
