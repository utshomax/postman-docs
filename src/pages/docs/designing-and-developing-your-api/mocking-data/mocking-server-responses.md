...
1. Select the new server stub and customize it for the server error scenario you want to simulate:
    ...
    * To rename the server stub, hover over the name, select the edit icon <img alt="Edit icon" src="https://assets.postman.com/postman-docs/documentation-edit-icon-v8-10.jpg#icon" width="18px">, and enter a new name.
...
Set an active server response to simulate different server error scenarios with your mock server. A mock server can have one active server response at a time. Your mock server will return the active server response for all requests made to the mock server.
...
A checkmark appears next to the server response to show it's active.
...
1. Make the server response active by turning on the **Set as active** toggle. (If the server response isn't set to active, you'll get a `404 Not Found` response or a scenario that matches the request.)
