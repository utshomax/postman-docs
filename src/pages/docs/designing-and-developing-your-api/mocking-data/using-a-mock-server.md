...
You can make calls to a mock server to simulate API interactions or endpoints for development and testing purposes. Use Postman, application code, or another tool to make calls to the mock server's endpoints. The mock server will respond with the data defined in the closest matching scenario.

You can configure the mock server to control access and customize its response to requests. You can also customize how the mock server responds by adding optional request headers. After making calls to the mock server, use the call logs to view details about incoming requests and outgoing responses.
...
* The available endpoints are listed under **Endpoint Stubs**. Each endpoint stub can have one or more scenarios, representing different ways of using an endpoint. Learn more about [endpoint stubs and scenarios](/docs/designing-and-developing-your-api/mocking-data/mocking-endpoints/).
...
To make a call to a mock server endpoint, add the endpoint path to the end of the mock server URL. Then send the request using the appropriate HTTP method. For example: `GET https://<mock-server-url>/users`
...
When you make a call, the mock server returns a response based on the scenario that best matches the request. For example, under the `/users` endpoint stub, you can have two different scenarios, `/users?id=123` and `/users?id=456`. If you send a request to `https://<mock-server-url>/users?id=123`, the mock server returns the response defined in the `/users?id=123` scenario. Learn more about [how mock servers match scenarios](/docs/designing-and-developing-your-api/mocking-data/matching-algorithm/).

### Mock server usage limits

Your [Postman plan](https://www.postman.com/pricing/) gives you a limited number of mock server calls per month. View your [Postman billing dashboard](https://go.postman.co/usage) to view your usage or purchase more blocks of mock server calls.
...
1. Change any of the configuration options:
    ...
    * **Environment** - Select the Postman [environment](/docs/sending-requests/managing-environments/) for the mock server to use. The mock server uses this environment to resolve any variables used in your scenarios.
...
Use the header `x-mock-response-code` to specify the HTTP response status code the returned response will match. For example, `500` will return a scenario (example)  with the 500 status code.

### Matching a response name or ID

Use the headers `x-mock-response-name` or `x-mock-response-id` to specify the exact response you want the mock server to return by matching the `id` or the `name` of the saved example.

You can get the example response `id` or `name` by using the Postman API to [GET a Single Collection](https://documenter.getpostman.com/view/12959542/UV5XjJV8?&_ga=2.100400478.1771040895.1644854022-1154140310.1627600155#a6a282df-907e-438b-8fe6-e5efaa60b8bf) and searching for your example in the response. You can also get the example `id` in Postman by selecting the information icon <img alt="Information icon" src="https://assets.postman.com/postman-docs/icon-information-v9-5.jpg#icon" width="16px"> in the right sidebar.

<img alt="Getting an example's ID" src="https://assets.postman.com/postman-docs/mock-example-detail-v9-19.jpg" width="444px"/>
...
* To enable request body matching, include the header `x-mock-match-request-body` when sending a request to the mock server and set its value to `true`. The mock server will use the request’s body to return matching scenarios.

* To enable request header matching, include the header `x-mock-match-request-headers` when sending a request to the mock server. Set its value to a comma-separated string of header keys that you want to match against the saved examples. Header matching isn't case-sensitive.

<img alt="Matching request body and headers" src="https://assets.postman.com/postman-docs/mock-match-body-headers-v9-19.jpg"/>
