...
`then.function` | **Required**. The name of the function to use. You can use all Spectral core functions in Postman. [Custom functions](/docs/api-governance/configurable-rules/configuring-custom-functions/) are only supported in API Governance rules. For more information, see the [Spectral documentation](https://github.com/stoplightio/spectral/blob/develop/docs/reference/functions.md).
