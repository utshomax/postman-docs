...
`resolved` | <p>The way Spectral processes your rule to check `$ref` properties. Spectral can process your rule as a resolved or unresolved document. The `$ref` property is the reference identifier in the form of a URI that references other components in the OpenAPI document. The possible values are `true` (default) and `false`. These values can be used as follows:</p> <ul><li>`true` - The rule is processed as a resolved document. The reference identifier is replaced with the component the URI points to. This enables you to target the `$ref` property in your rule to check the component the URI points to.</li><li>`false` - The rule is processed as an unresolved document. The reference identifier remains a URI, and isn't replaced with the component it points to. This enables you to target the `$ref` property to check the presence of the `$ref` property and the URI value as a string.</li></ul>
