...
> You can hide the number of security warnings that Postman displays in the **Security** tab if this number is not important to you. In the Postman header, select the settings icon <img alt="Settings icon" src="https://assets.postman.com/postman-docs/icon-settings-v9.jpg#icon" width="16px">, then select **Settings > General** and turn off **Security Warnings count**. While the number of warnings will no longer display next to the **Security** tab, you can still select this tab to inspect any warnings.
